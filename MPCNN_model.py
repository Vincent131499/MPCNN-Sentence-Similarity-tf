...
"""
-------------------------------------------------
   File Name：     MPCNN_model
   Description :  MPCNN模型文件。其中输入x的shape:[batch_size, sentence_length, enbedding_size, 1].
   卷积核（参数W）的shape:[宽度，高度，通道数，卷积核数量]，偏置项（参数b）的shape:[卷积核数量]
   Author :       Stephen
   date：          2018/9/3
-------------------------------------------------
   Change Activity:
...
import sim_compu_utils as utils
...
class MPCNN(object):
    def __init__(self, num_classes, embedding_size, filter_sizes, num_filters, n_hidden,
        ...
        self.Wh = tf.Variable(tf.random_normal([274, n_hidden], stddev=0.01), name='Wh')
    ...
    def attention(self):
        ...
        for i in range(len(sent1_unstack)):
            ...
            for j in range(len(sent2_unstack)):
                dis = utils.compute_cosine_distance(sent1_unstack[i], sent2_unstack[j])
    ...
    def similarity_sentence_layer(self):
        ...
        with tf.name_scope("cal_dis_with_alg1"):
            for i in range(3):
                ...
                #按照每个维度计算max,mean,min三个值的相似度
                for k in range(self.num_filters[0]):
                    #计算两个tensor的距离，得到一个（batch_size, 2）的tensor，2表示余弦距离，L1距离
                    fea_h.append(utils.comU2(regM1[:, :, k], regM2[:, :, k]))
        """fea_h是一个长度为3*20=60的list，其中每个元素都是（batch_size,2）的tensor"""
        #self.fea_h = fea_h
        print('算法1应用于block_A成功！')
            ...
            for i in range(3):
                for j in range(len(self.filter_sizes)):
                    for k in range(len(self.filter_sizes)):
                        fea_a.append(utils.comU1(sent1[i][j][:, 0, :], sent2[i][k][:, 0, :]))
        """fea_a是一个长度为3*3*3=27的list，其中每个元素都是（batch_size,3）的tensor"""
        print('算法2应用于block_A成功！')

        #sent1/sent2都是2*2*[batch_size, 50, num_filters_B]的嵌套列表
            ...
            for i in range(len(self.poolings)-1):
                for j in range(len(self.filter_sizes)-1):
                    for k in range(self.num_filters[1]):
                        fea_b.append(utils.comU1(sent1[i][j][:, :, k], sent2[i][j][:, :, k]))
        """fea_b是一个长度为2*2*20=80的list，其中每个元素都是（batch_size,3）的tensor"""
        #self.fea_b = fea_b
        print('算法2应用于block_B成功！')
